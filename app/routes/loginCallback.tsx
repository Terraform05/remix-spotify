...
export default function LoginCallback() {
  ...
  useEffect(() => {
    ...
    if (code && code_verifier !== null) {
      ...
      requestTokens(code, code_verifier)
        .then(({ access_token, refresh_token }) => {
          ...
          console.log('access_token after storage:', window.localStorage.getItem('access_token')); //logging
          console.log('refresh_token after storage:', window.localStorage.getItem('refresh_token')); //logging
          console.log('token_time after storage:', window.localStorage.getItem('token_time')); //logging
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', lineHeight: '1.8' }}>
      ...
      <ul>
        ...
        <li>token_time: {tokenTime ? new Date(parseInt(tokenTime)).toLocaleString() : ''}</li>
      </ul>
    </div>
  );
}
